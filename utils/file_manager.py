...
class FileManager(QObject):
    ...
    def open_file(self, parent_widget):
        file_path, _ = QFileDialog.getOpenFileName(
            parent_widget, "Open Image File", "", self.get_file_filter()
        )
        ...
        if file_path:
            try:
                ...
                QMessageBox.critical(
                    parent_widget, "Error", f"Failed to open file:\n{str(e)}"
                )
                return None, None
    ...
    def save_file_as(self, parent_widget, image_data):
        file_path, _ = QFileDialog.getSaveFileName(
            parent_widget, "Save Image File", "", self.get_file_filter()
        )
    ...
    def _save_to_path(self, parent_widget, image_data, file_path):
        if image_data is None:
            QMessageBox.warning(parent_widget, "Warning", "No image to save")
            return False
        ...
        try:
            ...
            QMessageBox.critical(
                parent_widget, "Error", f"Failed to save file:\n{str(e)}"
            )
            return False
