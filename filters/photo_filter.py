...
class PhotoFilter(BaseFilter):
    ...
    def apply(self, image: np.ndarray, **params) -> np.ndarray:
        ...
        preserve_luminosity = params.get(
            "preserve_luminosity", self._default_params["preserve_luminosity"]
        )
        ...
        if "filter_color" in params:
            ...
            filter_color = self.PRESET_COLORS.get(
                filter_name, self.PRESET_COLORS["Warming Filter (85)"]
            )

        # Photo Filter 알고리즘 적용
        return self._apply_photo_filter(
            image, filter_color, density, preserve_luminosity
        )
    ...
    def _apply_photo_filter(
        ...
        if preserve_luminosity:
            ...
            filtered_bgr = cv2.cvtColor(
                (filtered * 255).astype(np.uint8), cv2.COLOR_RGB2BGR
            )
