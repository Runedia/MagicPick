...
class FilterPipeline(QObject):
    ...
    def add_filter(self, filter_obj: BaseFilter, params: Optional[Dict[str, Any]] = None):
        """
        파이프라인에 필터를 추가합니다
...
class FilterManager(QObject):
    ...
    def apply_filter(self, image: np.ndarray, filter_name: str, **params) -> Optional[np.ndarray]:
        """
        이미지에 필터를 적용합니다
