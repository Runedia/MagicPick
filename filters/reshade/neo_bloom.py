...
class NeoBloomFilter(BaseFilter):
    ...
    def _blend_bloom(self, color, bloom):
        ...
        if self.blend_mode == 0:  # Mix
            return np.clip(
                color * (1.0 - np.log2(w + 1.0)) + bloom * np.log2(w + 1.0), 0, None
            )
    ...
    def _downsample_preprocess(self, img):
        ...
        for y in range(new_h):
            for x in range(new_w):
                result[y, x] = apply_saturation(result[y, x], self.saturation)
    ...
    def _create_bloom_atlas(self, downsampled):
        ...
        for i, (rect_x, rect_y, rect_scale, mip_level) in enumerate(self.bloom_levels):
    ...
    def _join_blooms(self, atlas, atlas_h, atlas_w):
        ...
        for y in range(atlas_h):
            for x in range(atlas_w):
                ...
                for i, (rect_x, rect_y, rect_scale, _) in enumerate(self.bloom_levels):
    ...
    def apply(self, image: np.ndarray, **params) -> np.ndarray:
        ...
        for y in range(orig_h):
            for x in range(orig_w):
                result[y, x] = self._blend_bloom(color_hdr[y, x], bloom_full[y, x])
