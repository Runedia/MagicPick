...
def _sharp_contrast_kernel(
    img, h, w, strength, edge_bias, edge_floor, gamma_correct, size
):
    ...
    if size <= 2:
        ...
        sector_count = 13  # HLSL says 13 for 10, 15 for >= 11
    if size >= 11:
        sector_count = 15
    ...
    for y in prange(h):
        for x in range(w):
            ...
            for dy in range(-radius, radius + 1):
                for dx in range(-radius, radius + 1):
                    ...
                    if dy == 0 and dx == 0:
                        ...
                        sector_idx = (
                            int((angle * sector_count) / (PI * 2)) % sector_count
                        )
            ...
            # Edge Multiplier
            edge_mult = (
                max((1.0 - (maximum - minimum) / maximum), 1e-5)
                if maximum > 0
                else 1e-5
            )  # rcp(maximum)
            ...
            for k in range(sector_count):
                if sample_count[k] > 0:
                    ...
                    variance = (
                        squared_sum[k] - (sum_val[k] * sum_val[k] / sample_count[k])
                    ) / sample_count[k]
                    ...
                    w_val = (
                        w_val * w_val
                    )  # power of 4 effectively? HLSL: weight *= weight; weight *= weight;
            ...
            final_val = (
                center_val + (center_val - kuwahara) * strength * edge_mult * 1.5
            )
