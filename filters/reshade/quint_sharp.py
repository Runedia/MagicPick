...
def quint_sharp_kernel(img, h, w, strength, use_rms, sharpen_mode):
    ...
    for y in prange(h):
        for x in range(w):
            ...
            # Overlay blend
            result[y, x, 0] = min(max(blend_overlay(E_r, 0.5 + sharpen_r), 0.0), 1.0)
            result[y, x, 1] = min(max(blend_overlay(E_g, 0.5 + sharpen_g), 0.0), 1.0)
            result[y, x, 2] = min(max(blend_overlay(E_b, 0.5 + sharpen_b), 0.0), 1.0)
...
class QUINTSharpFilter(BaseFilter):
    ...
    def apply(self, image: np.ndarray, **params) -> np.ndarray:
        ...
        self.rms_mask_enable = params.get("rms_mask_enable", self.rms_mask_enable)
