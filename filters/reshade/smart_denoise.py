        ...
        for x in range(w):
            ...
            for dx in range(-radius, radius + 1):
                ...
                for dy in range(-pt, pt + 1):
                    ...
                    delta_factor = (
                        np.exp(-dc_sq * inv_threshold_sqx2) * inv_threshold_sqrt2_pi *
                        np.exp(-d_sq * inv_sigma_qx2) * inv_sigma_qx2_pi
                    )
...
class SmartDeNoiseFilter(BaseFilter):
    ...
    def apply(self, image: np.ndarray, **params) -> np.ndarray:
        ...
        result = _smart_denoise_kernel(
            img_float, h, w, radius,
            inv_sigma_qx2, inv_sigma_qx2_pi,
            inv_threshold_sqx2, inv_threshold_sqrt2_pi
        )
