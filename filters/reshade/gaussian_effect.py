...
def _gaussian_passes(
    img, h, w, use_bp, bloom_intensity, use_h, use_v, use_s, passes, hw, vw, sw
):
    # 1. Bright Pass
    ...
    offsets = np.array(
        [0.0, 1.4347826, 3.3478260, 5.2608695, 7.1739130], dtype=np.float32
    )
    weights = np.array(
        [0.16818994, 0.27276957, 0.11690125, 0.024067905, 0.0021112196],
        dtype=np.float32,
    )
        ...
        for y in prange(h):
            for x in range(w):
                ...
                for i in range(1, min(passes, 5)):
                    ...
                    off_y = offsets[
                        i
                    ]  # Slant uses 1.0 for Y scale in shader? "sampleOffsets[i] * PIXEL_SIZE.y"
...
@njit(parallel=True, fastmath=True, cache=True)
def _gaussian_combine(
    img, blur, h, w, effect, strength, add_bloom, bloom_strength, bloom_warmth
):
    ...
    for y in prange(h):
        for x in range(w):
            ...
            if effect == 0:  # Off
                ...
                sl = (
                    sharp_r * sharp_strength_luma[0]
                    + sharp_g * sharp_strength_luma[1]
                    + sharp_b * sharp_strength_luma[2]
                )
                ...
                if bloom_warmth == 0:  # Neutral
                    ...
                    res_r = lerp(
                        orig_r, max(orig_r * 1.8 + blur_r * 5.0 - 1.0, 0.0), strength
                    )
                    res_g = lerp(
                        orig_g, max(orig_g * 1.8 + blur_g * 5.0 - 1.0, 0.0), strength
                    )
                    res_b = lerp(
                        orig_b, max(orig_b * 1.8 + blur_b * 5.0 - 1.0, 0.0), strength
                    )
                else:  # Foggy
                    res_r = lerp(
                        orig_r, 1.0 - (1.0 - orig_r) * (1.0 - blur_r), strength
                    )
                    res_g = lerp(
                        orig_g, 1.0 - (1.0 - orig_g) * (1.0 - blur_g), strength
                    )
                    res_b = lerp(
                        orig_b, 1.0 - (1.0 - orig_b) * (1.0 - blur_b), strength
                    )
...
class GaussianEffectFilter(BaseFilter):
    ...
    def apply(self, image: np.ndarray, **params) -> np.ndarray:
        ...
        blur = _gaussian_passes(
            ...
            True,
            True,
            True,  # Use all passes by default or configurable? Shader has #defines. Assuming all ON.
            self.passes,
            self.hw,
            self.vw,
            self.sw,
