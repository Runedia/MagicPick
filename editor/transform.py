...
class ImageTransform:
    ...
    @staticmethod
    def resize(
        image: np.ndarray, width: int, height: int, maintain_aspect: bool = False
    ) -> np.ndarray:
        """
        이미지 크기 조정
    ...
    @staticmethod
    def rotate_custom(
        image: np.ndarray, angle: float, expand: bool = True
    ) -> np.ndarray:
        """
        이미지를 임의 각도로 회전
        ...
        rotated = pil_image.rotate(
            angle, expand=expand, resample=Image.Resampling.BICUBIC
        )
