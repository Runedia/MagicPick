"""
빈티지 효과 조정 다이얼로그

사용자가 빈티지 효과 설정을 조정할 수 있는 다이얼로그입니다.
"""

import numpy as np

from filters.artistic import VintageFilter
from ui.dialogs.base_filter_dialog import BaseFilterDialog


class VintageDialog(BaseFilterDialog):
    """빈티지 효과 조정 다이얼로그"""

    def __init__(self, original_image: np.ndarray, parent=None):
        """
        초기화

        Args:
            original_image: 원본 이미지 (NumPy array, RGB 형식)
            parent: 부모 위젯
        """
        super().__init__(original_image, parent)
        self.setWindowTitle("빈티지 효과")

    def create_filter(self):
        """VintageFilter 인스턴스 생성"""
        return VintageFilter()

    def build_parameter_ui(self, layout):
        """파라미터 UI 구성"""
        # 세피아 강도 슬라이더
        sepia_group, self.sepia_slider, _ = self._create_slider_with_label(
            "세피아 강도", 0, 100, 70, lambda v: self.apply_filter(), suffix="%"
        )
        layout.addWidget(sepia_group)

        # 비네팅 강도 슬라이더
        vignette_group, self.vignette_slider, _ = self._create_slider_with_label(
            "비네팅 강도", 0, 100, 50, lambda v: self.apply_filter(), suffix="%"
        )
        layout.addWidget(vignette_group)

        # 그레인 강도 슬라이더
        grain_group, self.grain_slider, _ = self._create_slider_with_label(
            "그레인 강도", 0, 50, 15, lambda v: self.apply_filter()
        )
        layout.addWidget(grain_group)

        # 페이드 슬라이더
        fade_group, self.fade_slider, _ = self._create_slider_with_label(
            "페이드", 0, 100, 20, lambda v: self.apply_filter(), suffix="%"
        )
        layout.addWidget(fade_group)

    def get_filter_params(self):
        """필터 파라미터 반환"""
        return {
            "sepia_intensity": self.sepia_slider.value() / 100.0,
            "vignette_strength": self.vignette_slider.value() / 100.0,
            "grain_intensity": self.grain_slider.value(),
            "fade_amount": self.fade_slider.value() / 100.0,
        }
